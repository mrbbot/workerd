...
public:
  ...
  public:
    RetrievedBucket(
        R2Bucket::FeatureFlags featureFlags, uint subrequestChannel, kj::String name,
        kj::Date created)
      : R2Bucket(featureFlags, subrequestChannel, kj::mv(name), R2Bucket::friend_tag_t{}),
        created(created) {}

    kj::String getName() const { return kj::str(KJ_ASSERT_NONNULL(adminBucketName())); }
    kj::Date getCreated() const { return created; }
