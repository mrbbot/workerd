...
kj::StringPtr getEncodingId(Encoding encoding) {
  switch (encoding) {
    case Encoding::INVALID: return "invalid"_kj;
#define V(name, id) case Encoding::name: return id##_kj;
    EW_ENCODINGS(V)
#undef V
  }
  KJ_UNREACHABLE;
...
Decoder& TextDecoder::getImpl() {
  KJ_SWITCH_ONEOF(decoder) {
    KJ_CASE_ONEOF(dec, AsciiDecoder) { return dec; }
    KJ_CASE_ONEOF(dec, IcuDecoder) { return dec; }
  }
  KJ_UNREACHABLE;
}
