...
#ifdef _WIN32
thread_local std::atomic<ThreadProgressCounter*> activeProgressCounter(nullptr);
#else
...
ThreadProgressCounter::ThreadProgressCounter(std::atomic_uint64_t& counter)
    : savedValue(counter.load(std::memory_order_relaxed)),
      counter(counter) {
  if (activeProgressCounter.load(std::memory_order_relaxed) == nullptr) {
    // Release compiler barrier guarantees we're initialized before signal handlers can see us.
    std::atomic_signal_fence(std::memory_order_release);
    activeProgressCounter.store(this, std::memory_order_relaxed);
  } else {
...
ThreadProgressCounter::~ThreadProgressCounter() noexcept(false) {
  auto& self = KJ_ASSERT_NONNULL(activeProgressCounter.load(std::memory_order_relaxed),
  ...
  if (&self == this) {
    ...
    KJ_DEFER({
      activeProgressCounter.store(nullptr, std::memory_order_relaxed);
...
bool ThreadProgressCounter::hasProgress() {
  KJ_IF_MAYBE(progressCounter, activeProgressCounter.load(std::memory_order_relaxed)) {
    ...
    auto currentValue = progressCounter->counter.load(std::memory_order_relaxed);
...
void ThreadProgressCounter::acknowledgeProgress() {
  KJ_IF_MAYBE(progressCounter, activeProgressCounter.load(std::memory_order_relaxed)) {
    progressCounter->savedValue = progressCounter->counter.load(std::memory_order_relaxed);
  }
}
